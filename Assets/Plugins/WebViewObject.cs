...
//Add
#if UNITY_EDITOR_WIN
	GreeWebViewEditorWindow webView;
...
//Add
#if UNITY_EDITOR_WIN
		webView = GreeWebViewEditorWindow.Create<GreeWebViewEditorWindow>( "GreeEditorWebView", null, 200, 530, 800, 600 );
		webView.onCallFromJS = CallFromJS;
		webView.onLoaded = CallOnLoaded;
		webView.onError = CallOnError;
...
//Add
    ...
    {
#if UNITY_EDITOR_WIN
        //Å¶htmlÇ≈[href="Unity.call('click')"]ÇæÇ∆Ç±Ç»Ç¢
        //Å¶htmlÇ≈[href onclick="Unity.call('click');"]ÇæÇ∆Ç≠ÇÈ
        if (message.Equals("click"))
            webView.NotifyVisibility(false);
        //webView.Close();
#elif UNITY_ANDROID
            if (message.Equals("click"))
